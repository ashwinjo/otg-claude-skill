#!/usr/bin/env python3
"""
B2B Dataplane 10Gbps Throughput Test
=====================================
Test Name  : B2B Dataplane 10Gbps Throughput Test
Generated  : 2026-03-22
OTG Config : b2b_dataplane_10gbps.json (embedded)
...
Flows
-----
  port1_to_port2 : port1 TX -> port2 RX  @ 100% line rate, 64-byte Eth/IPv4/UDP, continuous
  port2_to_port1 : port2 TX -> port1 RX  @ 100% line rate, 64-byte Eth/IPv4/UDP, continuous
...
-------------------
  ...
  4. Stream port + flow metrics every 1 s for 60 s
  ...
  7. Run assertions on final snapshot:
       frames_tx > 0               (traffic was sent on both flows)
       frames_rx > 0               (traffic was received on both flows)
       loss_pct < 0.1              (max 0.1% frame loss tolerance)
       rx_rate_mbps > 0            (non-zero receive throughput)
       bidirectional traffic check (both flows active)
  8. Write JSON report to test_b2b_dataplane_report_<timestamp>.json
  9. Exit 0 on PASS, 1 on FAIL

Usage
-----
  pip install snappi
  python test_b2b_dataplane_10gbps.py
...
-----------------------------------
  - No latency metrics in OTG config (unsupported; see fixes.md: ixia-c-one Metric Field Incompatibility)
  - No loss metrics in OTG config (unsupported on ixia-c-one)
  - timestamps field stripped from flow metrics before push (not supported on all versions)
  - Continuous duration used — fixed_seconds crashes keng-controller (see fixes.md)
  - Traffic stopped programmatically via set_control_state after 60 s
  - cfg.deserialize() used, NOT cfg.loads() (see fixes.md: Snappi Config.loads() vs deserialize())
  - ControlState uses cs.traffic.flow_transmit.state, not snappi.ControlState.Protocol pattern
"""
...
log = logging.getLogger("b2b_dataplane_10gbps")
...
TEST_NAME: str = "B2B Dataplane 10Gbps Throughput Test"
...
LOSS_TOLERANCE_PCT: float = 0.1  # allow up to 0.1% frame loss (rounding tolerance)

# Port mapping (Docker Compose ixia-c B2B setup)
#   port1 -> localhost:5555  (TX flow1, RX flow2)
#   port2 -> localhost:5556  (TX flow2, RX flow1)

# OTG Configuration — derived from b2b_dataplane_10gbps.json
#
# Compatibility adjustments applied before embedding:
#   - Removed "timestamps" field from flow metrics (not supported on all ixia-c versions)
#   - Removed any latency/loss metric fields (unsupported on ixia-c-one)
#   - Duration kept as "continuous" (fixed_seconds crashes keng-controller — see fixes.md)
#   - metrics.enable kept as true (required for flow counter collection)
...
# Frame size for throughput calculation (64-byte frame + 20-byte Ethernet overhead = 84 bytes on wire)
FRAME_SIZE_BYTES: int = 64
WIRE_OVERHEAD_BYTES: int = 20       # preamble (7) + SFD (1) + IFG (12)
WIRE_FRAME_BYTES: int = FRAME_SIZE_BYTES + WIRE_OVERHEAD_BYTES  # 84 bytes per wire frame
...
def _push_config(api: snappi.Api) -> snappi.Config:
    ...
    snappi.Config; see fixes.md: Snappi Config.loads() vs deserialize()).
    """
...
def _start_traffic(api: snappi.Api) -> None:
    ...
    log.info("Starting traffic (all flows, 100%% line rate)...")
...
def _get_flow_metrics(api: snappi.Api) -> list[Any]:
    ...
    Uses req.choice = req.FLOW — not req.flow.state pattern which does not
    exist (see fixes.md: Snappi Script Generator — Fabricated Metrics API).
    """
...
def _poll_metrics(
    ...
    Prints a live table to stdout at each interval — output is not buffered
    so stats are visible in real-time.
...
def _run_assertions(
    ...
    Per flow (port1_to_port2, port2_to_port1):
      ...
      2. frames_rx > 0          (traffic was actually received — bidirectional check)
      3. loss_pct < 0.1         (max 0.1% loss tolerance for 64-byte line-rate traffic)
    ...
    for flow_name in flow_names:
        ...
        checks: list[dict[str, Any]] = [
            ...
            {
                "assertion": f"{flow_name}: loss_pct < {LOSS_TOLERANCE_PCT}% (max {LOSS_TOLERANCE_PCT}% tolerance)",
                "flow": flow_name,
                "metric": "loss_pct",
        ...
        for check in checks:
            ...
            log.info("[%s] %s  (actual: %s)", status, check["assertion"], check["actual"])
            if not check["passed"]:
                all_passed = False
...
def _save_report(data: dict[str, Any]) -> str:
    ...
    path = f"test_b2b_dataplane_report_{ts}.json"
...
def _print_throughput_summary(final_metrics: dict[str, Any]) -> None:
    """Print a human-readable throughput summary to stdout."""
    flows = final_metrics.get("flows", {})
    if not flows:
        return
    ...
    print("=" * 70)
    print(f"  {'Flow':<22s}  {'TX Frames':>12s}  {'RX Mbps':>10s}  {'RX Gbps':>10s}  {'Line%':>7s}")
    print("  " + "-" * 66)
...
def run_test() -> bool:
    """Execute the B2B 10Gbps dataplane test. Returns True if all assertions pass."""
    log.info("=" * 70)
    log.info("Test       : %s", TEST_NAME)
    log.info("Controller : %s", CONTROLLER_URL)
    log.info("Port map   : port1=localhost:5555  |  port2=localhost:5556")
    log.info("Speed      : %s Gbps  |  fiber  |  auto-negotiate=off", LINK_SPEED_GBPS)
    log.info(
        "Flows      : port1_to_port2 + port2_to_port1  |  100%% line rate  |  64-byte  |  %d s",
        TEST_DURATION_S,
    )
    log.info("=" * 70)
    ...
    try:
        ...
        # Print human-readable throughput table
    ...
    # -----------------------------------------------------------------------
    # Step 8: Write JSON report — always written, pass or fail
    # -----------------------------------------------------------------------
