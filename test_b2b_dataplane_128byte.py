#!/usr/bin/env python3
"""
Snappi Test Script: B2B Dataplane (128-byte, 1000 pps)
Generated for: Back-to-back bidirectional traffic testing
Controller: https://localhost:8443 (self-signed certificate)
Deployment: Docker Compose (ixia-c-te-a, ixia-c-te-z with veth-a/veth-z pair)
"""
...
OTG_CONFIG_JSON = r'''{
  "ports": [
    {"name": "port_a", "location": "veth-a"},
    {"name": "port_z", "location": "veth-z"}
  ],
  "flows": [
    {
      "name": "flow_a_to_z",
      ...
      "packet": [
        {
          "choice": "ethernet",
          "ethernet": {
            "src": {"choice": "value", "value": "00:00:01:00:00:01"},
            "dst": {"choice": "value", "value": "00:00:01:00:00:02"}
          }
        },
        {
          "choice": "ipv4",
      ...
      "packet": [
        {
          "choice": "ethernet",
          "ethernet": {
            "src": {"choice": "value", "value": "00:00:01:00:00:02"},
            "dst": {"choice": "value", "value": "00:00:01:00:00:01"}
          }
        },
        {
          "choice": "ipv4",
          "ipv4": {
            "src": {"choice": "value", "value": "10.0.0.2"},
            "dst": {"choice": "value", "value": "10.0.0.1"}
          }
        }
      ],
      "size": {"choice": "fixed", "fixed": 128},
...
def collect_metrics(api, flows, interval, duration):
    ...
    print("-" * 100)
    print(f"{'Time(s)':<10} | {'Flow':<16} | {'TxFrames':<15} | {'RxFrames':<15} | {'Loss%':<10}")
    print("-" * 100)
    ...
    try:
        while elapsed < duration:
            ...
            for flow_metric in resp.flow_metrics:
                ...
                print(f"{elapsed:<10} | {flow_name:<16} | {tx:<15} | {rx:<15} | {loss_pct:<10.2f}")
...
def generate_report(metrics_data):
    ...
    report_file = f"test_b2b_dataplane_128byte_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def main(auto_start=False):
    print("=" * 80)
    print("Snappi B2B Dataplane Test (128-byte, 1000 pps, bidirectional)")
    print("=" * 80)
    print(f"Controller: {API_LOCATION}")
    print(f"Duration: {TEST_DURATION} seconds")
    print(f"Ports: veth-a <-> veth-z")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Snappi B2B Dataplane Test (128-byte, 1000 pps)",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python3 test_b2b_dataplane_128byte.py              # Interactive mode
  python3 test_b2b_dataplane_128byte.py --auto-start # Auto-start (no prompt)
        """
    )
    parser.add_argument(
        "--auto-start",
